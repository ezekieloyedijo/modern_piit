;(function ($) {
    ...
    function startNupdate() {
      ...
      update_time = setInterval(function () {
        ...
        for (var i = 0; i < player.buffered.length; i++) {
          var perc_start = (
            (player.buffered.start(i) / player.duration) *
            100
          ).toString()
          var perc_end = (
            (player.buffered.end(i) / player.duration) *
            100
          ).toString()
  ...
  // Touch devices: allow tapping the About menu to toggle its submenu
  ;(function () {
    var isTouch = 'ontouchstart' in window || navigator.maxTouchPoints
    if (!isTouch) return
    function closeAllAbout() {
      document
        .querySelectorAll('.main-menu__list li.about-dropdown.open')
        .forEach(function (li) {
    ...
    document
      .querySelectorAll('.main-menu__list li.about-dropdown > a')
      .forEach(function (a) {
        a.addEventListener(
          'click',
          function (e) {
    ...
    document.addEventListener('click', function (e) {
      if (!e.target.closest('.main-menu__list li.about-dropdown')) {
        closeAllAbout()
      }
    })
    // adjust on resize
    window.addEventListener('resize', function () {
      if (window.matchMedia('(min-width: 992px)').matches) {
        document
          .querySelectorAll('.main-menu__list li.about-dropdown > ul')
          .forEach(function (ul) {
            ul.style.display = ''
          })
        document
          .querySelectorAll('.main-menu__list li.about-dropdown')
          .forEach(function (li) {
            li.classList.remove('open')
          })
      } else {
